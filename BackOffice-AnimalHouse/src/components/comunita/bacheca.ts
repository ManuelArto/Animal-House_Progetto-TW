	...
	fetch(ENDPOINT.THREADS_LIST)
		...
		.then((threads) => threads.forEach((thread: Thread) => {
			...
			product_tmpl.find("#title").attr("onClick", function() {
				table.hide();
				mex.show();
				initMessages(deleteModal, thread.title, thread.messages);
			})
...
function initMessages(deleteModal: Modal, title: string, messages: Message[]){
	var product_tmpl = $($("#message_template").html());
	product_tmpl.find("#title").text(title)

	messages.forEach((message:Message) => {
		product_tmpl.find(".deleteMessageButton").attr("id", `delete_${message._id}`)
		product_tmpl.find("#back").attr("onClick", function() {
			mex.hide();
			table.show();
		})
